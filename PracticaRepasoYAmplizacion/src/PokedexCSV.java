...
public class PokedexCSV {
    public PokedexCSV() throws FileNotFoundException {
    }

    static String solucioncc(String s){
...
// esto es para escribir en el csv
        try (PrintWriter es = new PrintWriter(
                new OutputStreamWriter
                        (new FileOutputStream("pokedex.csv"),
                                StandardCharsets.UTF_8))) {

            es.println("ID,Nombre,Tipo");
            for (Pokemon p : pokedex) {
                es.println(
                        solucioncc(String.valueOf(p.getId())) + ", "
                                + solucioncc(p.getNombre()) + ","
                                + solucioncc(p.getTipo())
                );
            }
...
// leer el csv anterior
        ...
        try( BufferedReader lc = new BufferedReader(
                new InputStreamReader
                        ( new FileInputStream("pokedex.csv"),
                                StandardCharsets.UTF_8)
        )){
            String linea;
            lc.readLine();
